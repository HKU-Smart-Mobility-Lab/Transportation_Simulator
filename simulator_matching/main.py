<<<<<<< HEAD:simulator_matching/main.py
            ...
            for cr_flag in cruise_flag:
                for single_driver_num in driver_num:
                    for single_max_distance_num in max_distance_num:
                        ...
                        if env_params['method'] == 'sarsa_no_subway':
                            matching_agent_params = {
                                'strategy_type': env_params['method'],
                                'strategy_params': qTable_params,
                                'load_path': load_path + '/episode_400/sarsa_q_value_table_epoch_400.pickle',
                                'flag_load': FLAG_LOAD  # 新增 FLAG_LOAD 参数
                            }
                        elif env_params['method']  == 'q_learning':
                            matching_agent_params = {
                                'strategy_type': env_params['method'],
                                'strategy_params': qTable_params,
                                'load_path': load_path + '/episode_200/Qlearning_q_value_table_epoch_200.pickle',
                                'flag_load': FLAG_LOAD  # 新增 FLAG_LOAD 参数
                            }
                        else:
                            matching_agent_params = {
                        ...
                        if env_params['rl_mode'] == "matching":
                            if simulator.experiment_mode == 'test':
                                ...
                                for num in range(last_stopping_index, test_num):
                                    ...
                                    #         load_path + '/episode_200/sarsa_q_value_table_epoch_200.pickle')
                                    # elif simulator.method in ['q_learning']:
                                    #     agent = QLearningAgent(**qTable_params)
                                    #     agent.load_parameters(
                                    #         load_path + '/episode_200/q_learning_q_value_table_epoch_200.pickle')
                                    ...
                                    for date in TEST_DATE_LIST:
                                        ...
                                        for step in range(simulator.finish_run_step):
                                            dispatch_transitions = simulator.rl_step()
                                            if step % 100 == 0:
                                ...
                                for epoch in range(NUM_EPOCH):
                                    ...
                                    start_time = time.time()
                                    for step in range(simulator.finish_run_step):
                                        # Andrew: epsilon isnt used in this function
                        ...
                        if env_params['rl_mode'] == "matching":
                            ...
                            if simulator.experiment_mode == 'train':
                                ...
                                pickle.dump(df, open(load_path + 'performance_record_test_' + env_params[
                                    'reposition_method'] + '.pickle', 'wb'))
                                print(df.iloc[num, :])
>>>>>>> 4c8765f (Description of your changes):simulator/main.py
