                ...
                # TODO: 后续对于不同的reward计算方式进行封装
                    ...
                    if self.method == 'instant_reward_no_subway':
                        ...
                        for i, (travel_time, reward, dest_grid_id) in enumerate(zip(
                            ...
                            print("DQN prediction", future_value)

                            weight_array[i] = reward + (qTable_params['discount_rate'] ** (end_time_slice - current_time_slice)) * future_value
                            self.transfer_request_num += 1
...
# rl_step for training: Andrew
    def rl_step_train(self, epsilon=0): # rl for matching
        ...
        if self.matching_agent is not None:
            print("Shape:", self.dispatch_transitions_buffer[0].shape, self.dispatch_transitions_buffer[1].shape, self.dispatch_transitions_buffer[2].shape, self.dispatch_transitions_buffer[3].shape)
